    ...
    def initialize_shop_items(conn: sqlite3.Connection):
        ...
        shop_items = [
            ...
            (7, "变大蘑菇", "item", 50, "Aeonreth", -1,
             "神秘的红帽子胡子大叔给你的蘑菇。使用后选择吃/不吃。\n💡使用指令：使用变大蘑菇", 1),
            (8, "缩小药水", "item", 50, "收养人", -1,
             "写着Drink Me的玻璃瓶。使用后选择喝/不喝，喝则下次投掷所有结果-1。\n💡使用指令：使用缩小药水", 1),
            (9, "超级大炮", "item", 200, "通用", -1,
             "外型凶猛的超级手持大炮。在任意回合掷骰前使用，可直接指定需要的出目(6个数字)。\n💡使用指令：使用超级大炮 1,2,3,4,5,6", 1),
            (10, ":）", "item", 100, "通用", -1,
             "一颗金色的星星。使用后选择互动/不互动，互动则本次移动的临时标记转换为永久标记且可继续当前轮次。\n💡使用指令：使用:）", 1),
            ...
            (16, "The Room", "item", 0, "通用", -1,
             "虚拟密闭空间，只有一次探索机会。选择正确位置可获得协会徽章，选择直接登顶。\n💡使用指令：使用The Room", 1),
